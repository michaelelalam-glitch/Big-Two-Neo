-- ============================================================================
-- Migration: fix_phase_bc_bot_multiplier
-- Branch: game/chinese-poker
-- Date: 2026-03-10
--
-- Addresses Copilot PR-113 review comments:
--
-- 1. Phase B ABANDONED stat writes (process_disconnected_players) now derive
--    and pass p_bot_multiplier so casual/private ELO penalties are scaled the
--    same way as the complete-game edge function.  Previously the multiplier
--    defaulted to 1.0 regardless of bot difficulty.
--
-- 2. Phase C ABANDONED stat writes have the same fix.
--
-- Derivation logic mirrors complete-game/index.ts: hardest bot_difficulty
-- present in room_players wins (hard→0.9, medium→0.7, easy→0.5, none→1.0).
-- ============================================================================
...
COMMENT ON FUNCTION public.process_disconnected_players IS
  ...
  'Phase B and Phase C ABANDONED stats now pass p_bot_multiplier derived from the '
