...
-- deltas, but existing data that already went negative stays negative, causing
-- slow painful recovery while new players start at 1000.
--
-- This migration does two things:
-- 1. One-time repair: clamp any existing negative casual_rank_points and
--    ranked_rank_points values up to 0 (zero), and sync rank_points (legacy
...
UPDATE player_stats
SET
  casual_rank_points  = GREATEST(0, COALESCE(casual_rank_points, 1000)),
  ranked_rank_points  = GREATEST(0, COALESCE(ranked_rank_points, 1000)),
  rank_points         = GREATEST(0, COALESCE(rank_points, 1000))
WHERE
  COALESCE(casual_rank_points, 0) < 0
  OR COALESCE(ranked_rank_points, 0) < 0
  OR COALESCE(rank_points, 0) < 0;

-- ──────────────────────────────────────────────────────────────────────────────
-- PART 2: Update the stats function to floor rank points at 0 going forward
-- ──────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ── Rank point change ────────────────────────────────────────────────────
  -- Casual: score-based formula, capped so completed games always give ≥ 0.
  -- Ranked: chess K=32 pairwise delta pre-computed by the edge function.
  -- Private: no rank impact.
  v_rank_point_change := CASE
    WHEN p_game_type = 'casual' THEN
      ROUND((100 - CASE WHEN p_completed THEN LEAST(p_score, 100) ELSE p_score END) * p_bot_multiplier)::INTEGER
    ELSE 0
  END;

  -- Floor casual rank points at 0: a player can never sink below 0 rank.
  -- GREATEST(0, ...) means abandoned-game penalties stop at 0 rather than going negative.
  ...
  -- Floor ranked ELO at 0 for the same reason.
