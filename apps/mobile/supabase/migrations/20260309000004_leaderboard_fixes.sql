...
BEGIN
  ...
  IF p_game_type = 'casual' THEN
    ...
    v_mode_completed   := COALESCE(v_stats.casual_games_completed, 0) + CASE WHEN p_completed THEN 1 ELSE 0 END;
    v_mode_total_cards := COALESCE(v_stats.casual_total_cards_left, 0) + p_cards_left;
    v_mode_avg_cards   := CASE WHEN (COALESCE(v_stats.casual_games_played, 0) + 1) > 0
                           THEN ROUND(v_mode_total_cards::DECIMAL / (COALESCE(v_stats.casual_games_played, 0) + 1)::DECIMAL, 2)
                           ELSE 0 END;
    v_mode_avg_score   := CASE WHEN p_completed AND v_mode_completed > 0
                           THEN ROUND(
    ...
    v_mode_avg_pos     := CASE WHEN p_completed AND v_mode_completed > 0
                           THEN ROUND(
    ...
    UPDATE player_stats SET
      ...
      casual_rank_points     = v_new_casual_rp,
      -- Completion
      casual_games_completed = COALESCE(casual_games_completed, 0) + CASE WHEN p_completed THEN 1 ELSE 0 END,
      ...
      -- Performance
      casual_total_points        = COALESCE(casual_total_points, 0) + CASE WHEN p_completed THEN p_score ELSE 0 END,
      casual_highest_score       = CASE WHEN p_completed THEN GREATEST(COALESCE(casual_highest_score, 0), p_score) ELSE COALESCE(casual_highest_score, 0) END,
      casual_lowest_score        = CASE WHEN p_completed THEN
      ...
      casual_avg_score_per_game  = v_mode_avg_score,
      casual_avg_finish_position = v_mode_avg_pos,
      casual_total_cards_left    = v_mode_total_cards,
      casual_avg_cards_left      = v_mode_avg_cards,
      -- Combos (only for completed games)
      casual_singles_played            = COALESCE(casual_singles_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'singles')::INTEGER, 0) ELSE 0 END,
      casual_pairs_played              = COALESCE(casual_pairs_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'pairs')::INTEGER, 0) ELSE 0 END,
      casual_triples_played            = COALESCE(casual_triples_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'triples')::INTEGER, 0) ELSE 0 END,
      casual_straights_played          = COALESCE(casual_straights_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'straights')::INTEGER, 0) ELSE 0 END,
      casual_flushes_played            = COALESCE(casual_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'flushes')::INTEGER, 0) ELSE 0 END,
      casual_full_houses_played        = COALESCE(casual_full_houses_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'full_houses')::INTEGER, 0) ELSE 0 END,
      casual_four_of_a_kinds_played    = COALESCE(casual_four_of_a_kinds_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'four_of_a_kinds')::INTEGER, 0) ELSE 0 END,
      casual_straight_flushes_played   = COALESCE(casual_straight_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'straight_flushes')::INTEGER, 0) ELSE 0 END,
      casual_royal_flushes_played      = COALESCE(casual_royal_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'royal_flushes')::INTEGER, 0) ELSE 0 END
    WHERE user_id = p_user_id;
  ...
  ELSIF p_game_type = 'ranked' THEN
    ...
    v_mode_completed   := COALESCE(v_stats.ranked_games_completed, 0) + CASE WHEN p_completed THEN 1 ELSE 0 END;
    v_mode_total_cards := COALESCE(v_stats.ranked_total_cards_left, 0) + p_cards_left;
    v_mode_avg_cards   := CASE WHEN (COALESCE(v_stats.ranked_games_played, 0) + 1) > 0
    ...
    UPDATE player_stats SET
      ...
      -- Completion
      ranked_games_completed = COALESCE(ranked_games_completed, 0) + CASE WHEN p_completed THEN 1 ELSE 0 END,
      ...
      -- Performance
      ranked_total_points        = COALESCE(ranked_total_points, 0) + CASE WHEN p_completed THEN p_score ELSE 0 END,
      ranked_highest_score       = CASE WHEN p_completed THEN GREATEST(COALESCE(ranked_highest_score, 0), p_score) ELSE COALESCE(ranked_highest_score, 0) END,
      ranked_lowest_score        = CASE WHEN p_completed THEN
      ...
      ranked_avg_score_per_game  = v_mode_avg_score,
      ranked_avg_finish_position = v_mode_avg_pos,
      ranked_total_cards_left    = v_mode_total_cards,
      ranked_avg_cards_left      = v_mode_avg_cards,
      -- Combos (only for completed games)
      ranked_singles_played            = COALESCE(ranked_singles_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'singles')::INTEGER, 0) ELSE 0 END,
      ranked_pairs_played              = COALESCE(ranked_pairs_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'pairs')::INTEGER, 0) ELSE 0 END,
      ranked_triples_played            = COALESCE(ranked_triples_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'triples')::INTEGER, 0) ELSE 0 END,
      ranked_straights_played          = COALESCE(ranked_straights_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'straights')::INTEGER, 0) ELSE 0 END,
      ranked_flushes_played            = COALESCE(ranked_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'flushes')::INTEGER, 0) ELSE 0 END,
      ranked_full_houses_played        = COALESCE(ranked_full_houses_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'full_houses')::INTEGER, 0) ELSE 0 END,
      ranked_four_of_a_kinds_played    = COALESCE(ranked_four_of_a_kinds_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'four_of_a_kinds')::INTEGER, 0) ELSE 0 END,
      ranked_straight_flushes_played   = COALESCE(ranked_straight_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'straight_flushes')::INTEGER, 0) ELSE 0 END,
      ranked_royal_flushes_played      = COALESCE(ranked_royal_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'royal_flushes')::INTEGER, 0) ELSE 0 END
    WHERE user_id = p_user_id;
  ...
  ELSIF p_game_type = 'private' THEN
    ...
    v_mode_completed   := COALESCE(v_stats.private_games_completed, 0) + CASE WHEN p_completed THEN 1 ELSE 0 END;
    v_mode_total_cards := COALESCE(v_stats.private_total_cards_left, 0) + p_cards_left;
    v_mode_avg_cards   := CASE WHEN (COALESCE(v_stats.private_games_played, 0) + 1) > 0
    ...
    UPDATE player_stats SET
      ...
      -- Completion
      private_games_completed = COALESCE(private_games_completed, 0) + CASE WHEN p_completed THEN 1 ELSE 0 END,
      ...
      -- Performance
      private_total_points        = COALESCE(private_total_points, 0) + CASE WHEN p_completed THEN p_score ELSE 0 END,
      private_highest_score       = CASE WHEN p_completed THEN GREATEST(COALESCE(private_highest_score, 0), p_score) ELSE COALESCE(private_highest_score, 0) END,
      private_lowest_score        = CASE WHEN p_completed THEN
      ...
      private_avg_score_per_game  = v_mode_avg_score,
      private_avg_finish_position = v_mode_avg_pos,
      private_total_cards_left    = v_mode_total_cards,
      private_avg_cards_left      = v_mode_avg_cards,
      -- Combos (only for completed games)
      private_singles_played            = COALESCE(private_singles_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'singles')::INTEGER, 0) ELSE 0 END,
      private_pairs_played              = COALESCE(private_pairs_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'pairs')::INTEGER, 0) ELSE 0 END,
      private_triples_played            = COALESCE(private_triples_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'triples')::INTEGER, 0) ELSE 0 END,
      private_straights_played          = COALESCE(private_straights_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'straights')::INTEGER, 0) ELSE 0 END,
      private_flushes_played            = COALESCE(private_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'flushes')::INTEGER, 0) ELSE 0 END,
      private_full_houses_played        = COALESCE(private_full_houses_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'full_houses')::INTEGER, 0) ELSE 0 END,
      private_four_of_a_kinds_played    = COALESCE(private_four_of_a_kinds_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'four_of_a_kinds')::INTEGER, 0) ELSE 0 END,
      private_straight_flushes_played   = COALESCE(private_straight_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'straight_flushes')::INTEGER, 0) ELSE 0 END,
      private_royal_flushes_played      = COALESCE(private_royal_flushes_played, 0) + CASE WHEN p_completed THEN COALESCE((p_combos_played->>'royal_flushes')::INTEGER, 0) ELSE 0 END
    WHERE user_id = p_user_id;
  END IF;
