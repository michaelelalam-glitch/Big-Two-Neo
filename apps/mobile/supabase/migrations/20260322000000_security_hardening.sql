...
-- The 2-arg overload was dropped in an early migration on some environments;
-- use a DO block so the ALTER is a no-op if the function doesn't exist.
DO $$ BEGIN
  ALTER FUNCTION public.start_game_with_bots(uuid, text) SET search_path = public, pg_catalog;
EXCEPTION WHEN undefined_function THEN
  RAISE NOTICE 'start_game_with_bots(uuid, text) not found — skipping';
END $$;
...
-- Read: all authenticated users may read all room_analytics rows.
-- room_analytics contains aggregate event data (not sensitive PII), so broad
-- authenticated access is intentional here. If per-room scoping is needed in
-- future, join against room_players/rooms with auth.uid().
