-- ============================================================================
-- Migration: fix_phase_b_lte_bot_replace
-- Branch: task/625-leave-in-game-modal-shows-home-banner
-- Date: 2026-03-11
--
-- Problem:
--   Phase B of process_disconnected_players() uses a strict less-than (<)
--   comparison for the 60-second bot-replacement threshold:
--
--     rp.disconnect_timer_started_at < NOW() - INTERVAL '60 seconds'
--
--   At the exact moment the client-side ring fires (elapsed = 60s), the server
--   evaluates T=disconnect < NOW()-60s = T=disconnect, which is FALSE (T = T).
--   This means the first forceSweep (triggered by the ring expiry) does NOT
--   replace the player.  The 1-second retry then catches it (T+1 seconds > T),
--   but adds an unnecessary 1-2 second window where the ring is gone but the
--   player has not yet been replaced.
--
-- Fix:
...
--
--   Combined with the client-side anchor fix (use rp.last_seen_at instead of
--   new Date() so the ring and Phase A share the same start timestamp), the
--   timing is now: disconnect → ring depletes 60s → ring expires → bot
--   replaces → Realtime event → UI updates.
--
-- This is a content-identical copy of 20260311000003_fix_phase_b_bot_fk.sql
-- with only the comparison operator changed on the Phase B WHERE clause.
-- ============================================================================
