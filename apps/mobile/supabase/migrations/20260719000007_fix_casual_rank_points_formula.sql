-- Migration: Fix casual rank point formula in update_player_stats_after_game
-- This is a complete replacement of the function, fixing three bugs vs migration
-- 20260719000006:
--
--   Bug 1 — LEAST(p_score, 100) cap (line ~54 of 000006):
--     Score ≥ 100 was artificially floored, preventing negative deltas for
--     high-scoring (poorly-performing) players. The correct formula has NO cap:
--       ROUND((100 - p_score) × bot_multiplier)
--     score 0   → +100×mult (winner)
--     score 100 → 0
--     score 101 → -1×mult (negative — expected for bad performance)
--     score 200 → -100×mult
--
--   Bug 2 — Abandoned penalty was -50 instead of -25 (line ~52 of 000006):
--     Tests specify a fixed -25 for all abandoned players regardless of score.
--
--   Bug 3 — No explicit voided guard in v_rank_point_change (line ~51 of 000006):
--     Voided players have p_voided=true AND p_completed=false, so they previously
--     also hit the abandoned branch and received -50 (though the subsequent UPDATE
--     protected actual casual_rank_points via CASE WHEN NOT p_voided). The side
--     effect was that rank_points_history received v_new_casual_rp = current − 50
--     for voided games, creating a false dip in the progression graph.
--
--   (No floor guard is applied — rank points can go negative and recover.)
--
-- Everything else (win-rate, avg-score, combo tracking, per-mode stats,
-- rank_points_history ORDER BY fix, SET search_path) is identical to 000006.
...
BEGIN
  ...
  -- ── FIXED: casual rank point delta ───────────────────────────────────────
  -- voided    → 0       (no change, same as test spec)
  -- abandoned → -25     (fixed penalty; was incorrectly -50)
  -- completed → ROUND((100 − score) × mult)  NO score cap (was LEAST(score,100))
  v_rank_point_change := CASE
    WHEN p_game_type = 'casual' THEN
      CASE
        WHEN p_voided        THEN 0
        WHEN NOT p_completed THEN -25
        ELSE ROUND((100 - p_score)::DECIMAL * p_bot_multiplier)::INTEGER
      END
    ELSE 0
  END;
  ...
  -- Wrap UNION ALL in a subquery so ORDER BY can use expressions without the
  -- "Only result column names can be used" PostgreSQL restriction.
  UPDATE player_stats SET
    rank_points_history = (
