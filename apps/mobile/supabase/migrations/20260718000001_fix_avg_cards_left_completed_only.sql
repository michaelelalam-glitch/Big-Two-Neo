-- Migration: fix avg_cards_left to only count completed, non-voided games
-- Previous implementation used (games_played + 1) as the denominator, which
-- inflated the divisor with abandoned/voided games and produced a lower-than-real
-- average. This migration corrects the denominator to (games_completed + 1) and
-- gates the running total on p_completed AND NOT p_voided for all three modes.
...
BEGIN
  ...
  -- FIX: use games_completed + 1 (not games_played + 1) so that abandoned and
  -- voided games do not dilute the average. Also fall back to the existing value
  -- rather than re-computing when the game does not count.
  ...
  UPDATE player_stats SET
    ...
    END,
    -- Cards left (global) — only completed, non-voided games count
    -- FIX: previously gated only on NOT p_voided, which included abandoned games
    -- and used games_played + 1 as denominator (inflating it).
  ...
  IF p_game_type = 'casual' THEN
    ...
    -- FIX: only add p_cards_left for completed, non-voided games; use v_mode_completed as denominator
