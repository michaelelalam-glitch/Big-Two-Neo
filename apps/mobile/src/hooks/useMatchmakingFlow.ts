...
export function useMatchmakingFlow(
  ...
  const handleRankedMatch = useCallback(async (retryCount = 0): Promise<void> => {
    const MAX_RETRIES = 3;
    if (!user || !profile) {
    ...
    try {
      ...
      const { error: cleanupError } = await supabase.from('room_players').delete().eq('user_id', user.id);
      if (cleanupError) roomLogger.error('⚠️ Cleanup warning:', cleanupError.message);

      await new Promise(resolve => setTimeout(resolve, 100));

      const { data: stillInRoom } = await supabase
        .from('room_players').select('room_id').eq('user_id', user.id).maybeSingle();
      if (stillInRoom) {
        ...
        for (const room of availableRooms) {
          ...
          if (count !== null && count < 4) {
            ...
            const { error: joinError } = await supabase.rpc('join_room_atomic', {
              p_room_code: room.code, p_user_id: user.id, p_username: username,
            });
            if (joinError) { roomLogger.error(`❌ Failed to join ${room.code}:`, joinError.message); continue; }
      ...
      const { data: roomResult, error: createError } = await supabase.rpc('get_or_create_room', {
        p_user_id: user.id, p_username: username,
        p_is_public: true, p_is_matchmaking: true, p_ranked_mode: true,
      });
      if (createError) {
      ...
      if (!result || !result.success || !result.room_code) throw new Error('Failed to create ranked room');
  ...
  const handleQuickPlay = useCallback(async (retryCount = 0): Promise<void> => {
    const MAX_RETRIES = 3;
    if (!user || !profile) {
    ...
    try {
      ...
      const { data: availableRooms, error: searchError } = await supabase
        ...
        .order('created_at', { ascending: true })
        .limit(5);
      if (searchError) { roomLogger.error('❌ Search error:', searchError); throw searchError; }
      ...
      const { data: roomResult, error: createError } = await supabase.rpc('get_or_create_room', {
        p_user_id: user.id, p_username: username,
        p_is_public: true, p_is_matchmaking: true, p_ranked_mode: false,
      });
      if (createError) {
  ...
  const handleStartOfflineWithDifficulty = useCallback((difficulty: 'easy' | 'medium' | 'hard') => {
