...
- [x] **C4** — Remove `.bak` files from version control
  ...
  - **Status:** `.gitignore` already contains `*.bak`. No further action required.
...
- [x] **C6** — Fix room connect timeout on rejoin after force-close
  ...
  - **Branch:** `task/fix-dedup-stats-reconnect`
  - **Fix:** Added retry logic (up to 3 attempts with exponential backoff: 1s, 2s, 4s) to the `multiplayerConnectToRoom` call in the mount `useEffect`. On cold app start, Supabase connection may not be ready yet, causing the 5s timeout to fire before the room query returns.
...
- [x] **H1** — Extract disconnect logic into a dedicated hook/reducer
  - **File:** `src/screens/MultiplayerGame.tsx` lines 580–842 → `src/hooks/useDisconnectDetection.ts`
  - **Task:** #633
  - **Branch:** `task/633-extract-disconnect-state-machine`
  - **Fix:** Created `src/hooks/useDisconnectDetection.ts` with a `useReducer`-based disconnect map (explicit `SEED / CORRECT / CLEAR / REPLACE` action types). Extracted all 263 lines of disconnect logic from `MultiplayerGame.tsx`:
    - `disconnectMapReducer` — pure reducer with built-in equality check (prevents re-renders when map contents haven't changed).
    - 1 s stale-heartbeat detection interval, immediate-reconnect-clear effect, game-state ref sync, turn-ring continuity refs (`lastTurnStartedAtRef`, `localPlayerWasActiveRef`), and sweep-retry timeout cleanup.
    - `handleLocalPlayerCountdownExpired` / `handleOtherPlayerDisconnectExpired` callbacks.
    - `enrichedLayoutPlayers` useMemo.
    - Exported `LayoutPlayerWithScore` from `usePlayerDisplayData.ts` to enable the `EnrichedLayoutPlayer` output type.
  - `MultiplayerGame.tsx` disconnect section reduced to 15 lines (single `useDisconnectDetection(...)` call).  Previous 263-line multi-level `useEffect` / `useMemo` block is fully replaced.
  - TypeScript: zero errors (`npx tsc --noEmit`).
  - **Why:** 263-line `useEffect` with 6+ nesting levels was untestable and error-prone; reducer-based hook allows unit testing each transition in isolation.
