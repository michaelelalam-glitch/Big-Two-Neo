...
- [x] **#22 🟠 P14-2** — No RLS policy tests in CI — a migration mistake could silently expose player data.  
  **Fix:** Add `supabase db test` or `pgTAP` tests covering each table's RLS policies (select, insert, update, delete).  
  ...
  > ✅ Fixed in PR (Tier 4) — SQL test file created at `apps/mobile/supabase/tests/rls_policies.sql` with 26 assertions covering 9 tables: `profiles`, `rooms`, `room_players`, `player_stats`, `rate_limit_tracking`, `blocked_users`, `game_history`, `waiting_room`, `bot_coordinator_locks`. (`push_tokens` and `friendships` excluded — not present in CLI-managed migrations.) `supabase test db` uses the Supabase test runner's built-in pgTAP support for local runs; in CI the command targets the hosted database via `--db-url`, so the target DB must have the `pgtap` extension enabled. CI step gated on `SUPABASE_DB_PASSWORD`; fails when present so RLS regressions are caught.
